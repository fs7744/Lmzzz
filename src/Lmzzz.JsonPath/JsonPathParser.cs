...
public class JsonPathParser
{
    ...
    public static Parser<IStatement> SliceSelector = Optional(Start.And(S)).And(Char(':')).And(S).And(Optional(End.And(S))).And(Optional(Char(':').And(Optional(S.And(Step)))));
    ...
    {
        Operater = x.Item1.ToString(),
        Statement = x.Item3
    });

    public static Parser<IStatement> LogicalExpr = LogicalOrExpr;
    public static Parser<IStatement> LogicalOrExpr = LogicalAndExpr.And(ZeroOrMany(S.And(Text("||")).And(S).And(LogicalAndExpr)));
    public static Parser<IStatement> LogicalAndExpr = BasicExpr.And(ZeroOrMany(S.And(Text("&&")).And(S).And(BasicExpr)));
    ...
    public static Parser<IStatement> ParenExpr = Optional(LogicalNotOp.And(S)).And(Char('(')).And(S).And(LogicalExpr).And(S).And(Char(')'))
        ...
        {
            Operater = x.Item1.Item1.ToString(),
            Statement = x.Item4
        });

    public static Parser<char> LogicalNotOp = Char('!');
    public static Parser<char> TestExpr = Optional(LogicalNotOp.And(S)).And(FilterQuery.Or(FunctionExpr)).Then<IStatement>(static x => new UnaryOperaterStatement() { Operater = x.Item1.Item1.ToString(), Statement = x.Item2 });
    ...
    public static Parser<IStatement> RelSingularQuery = CurrentNodeIdentifier.And(SingularQuerySegments);
    public static Parser<IStatement> AbsSingularQuery = RootIdentifier.And(SingularQuerySegments);
    public static Parser<IStatement> SingularQuerySegments = ZeroOrMany(S.And(NameSegment.Or(IndexSegment)));
    public static Parser<IStatement> NameSegment = Char('[').And(NameSelector).And(Char(']')).Or(Char('.').And(MemberNameShorthand));
    ...
    public static Parser<IStatement> FunctionExpr = FunctionName.And(Char('(')).And(S).And(Optional(FunctionArgument.And(ZeroOrMany(S.And(Char(',')).And(S).And(FunctionArgument))))).And(S).And(Char(')'));
    ...
    public static Parser<char> DescendantSegment = Char('.').And(Char('.')).And(BracketedSelection.Or(WildcardSelector).Or(MemberNameShorthand));
}