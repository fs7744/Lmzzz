...
public class RequestQueryGenericHttpContextFieldConvertor : GenericHttpContextFieldConvertor
{
    public override string Key()
    {
        return "field_Request.Query";
    }
    ...
    public override bool TryConvertStringFunc(IStatement statement, out Func<HttpContext, string> func)
    {
        if (statement is FieldStatement field)
        {
            ...
            {
                func = c => c.Request.Query.ToString();
                return true;
            }
            else if (field.Names.Count >= 3)
            {
                ...
                {
                    func = c => c.Request.Query[k].ToString();
                    return true;
                }
    ...
    public override IStatement ConvertFieldStatement(FieldStatement field)
    {
        if (field.Names.Count == 2)
        {
            return new HttpTemplateFuncFieldStatement(field.Names, static c => c.Request.Query);
        }
        else if (field.Names.Count >= 3)
        {
            var k = field.Names[2];
            if (field.Names.Count == 3)
            {
                return new HttpTemplateFuncFieldStatement(field.Names, c => c.Request.Query[k]);
            }
            else if (field.Names.Count == 4 && int.TryParse(field.Names.Last(), out var i) && i >= 0)
            {
            ...
            {
                return new HttpTemplateFuncFieldStatement(field.Names, c => c.Request.Query[k].Count);
            }
            else
            {
                var f = FieldStatement.CreateGetter(field.Names.Skip(3), Template.FieldStatementMode.Defined);
                return new HttpTemplateFuncFieldStatement(field.Names, c => f(c.Request.Query[k]));
            }
        }
    ...
    public override IHttpConditionStatement GenericConvertEqual(FieldStatement field, IStatement statement, Dictionary<string, HttpTemplateFuncFieldStatement> fields)
    {
        if (field.Names.Count == 2)
        {
            ...
            if (s is IObjectHttpStatement o)
                return new ActionConditionStatement(c => EqualStatement.Eqs(c.Request.Query, o.EvaluateObjectHttp(c)));
        }
        else if (field.Names.Count >= 3)
        {
            var k = field.Names[2];
            if (field.Names.Count == 3)
            {
                ...
                {
                    return new ActionConditionStatement(c => c.Request.Query[k].ToString() == fs);
                }

                if (DefaultTemplateEngineFactory.TryGetStringFunc(statement, out var f))
                {
                    return new ActionConditionStatement(c => c.Request.Query[k].ToString() == f(c));
                }

                var s = DefaultTemplateEngineFactory.OptimizeTemplateEngine(statement, fields);
                if (s is IObjectHttpStatement o)
                    return new ActionConditionStatement(c => EqualStatement.Eqs(c.Request.Query[k].ToString(), o.EvaluateObjectHttp(c)));
            }
            else if (field.Names.Count == 4 && int.TryParse(field.Names.Last(), out var i) && i >= 0)
            {
                if (DefaultTemplateEngineFactory.TryGetString(statement, out var fs))
            ...
            else if (field.Names.Count == 4 && "Count".Equals(field.Names.Last(), StringComparison.OrdinalIgnoreCase))
            {
                ...
                {
                    return new ActionConditionStatement(c => c.Request.Query[k].Count == d);
                }
            }
            else
            {
                ...
                {
                    var f = FieldStatement.CreateGetter(field.Names.Skip(3), Template.FieldStatementMode.Defined);
                    return new ActionConditionStatement(c => EqualStatement.Eqs(f(c.Request.Query[k]), o.EvaluateObjectHttp(c)));
                }
            }
        }
