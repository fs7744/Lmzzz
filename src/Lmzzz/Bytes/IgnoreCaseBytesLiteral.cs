...
public class IgnoreCaseBytesLiteral : ByteParser<ReadOnlySequence<byte>>
{
    ...
    public static Func<ReadOnlySequence<byte>, bool> BuildIgnoreCaseFunc(string str, Encoding encoding)
    {
        ...
        var all = Enumerable.Range(0, length).Select<int, (int, Func<ReadOnlySequence<byte>, bool>)>(i =>
        {
            var ll = l[i];
            var uu = u[i];
            if (ll == uu)
            {
                var b = encoding.GetBytes(ll.ToString());
                return (b.Length, (ReadOnlySequence<byte> s) => s.StartsWith(b));
            }
            else
            {
                var bl = encoding.GetBytes(ll.ToString());
                var bu = encoding.GetBytes(uu.ToString());
                return (bl.Length, (ReadOnlySequence<byte> s) => s.StartsWith(bl) || s.StartsWith(bu));
            }
        }).ToArray();
